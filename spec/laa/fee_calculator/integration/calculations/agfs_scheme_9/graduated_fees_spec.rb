        ...
        context 'with a Trial' do
          ...
          describe 'units' do
            describe 'days' do
              ...
              context 'with 3 to 40 days' do
                agfs_scheme_9_trial_fees_and_uplifts.each do |row|
                  describe "Daily attendance for #{row[:advocate_type]} and offence #{row[:offence_class]}" do
                    ...
                    [3, 40].each do |attendance|
                      context "with #{attendance} days" do
                        let(:days) { attendance }
              ...
              context 'with upper limit 9,999' do
                [9_999, 10_000].each do |quantity|
                  context "with #{quantity} days" do
                    let(:days) { quantity }

                    it 'returns max. amount' do
                      expect(calculate).to be 2_859_897.0
                    end
                  end
                end
              end
            ...
            describe 'pages of prosecution evidence (ppe)' do
              let(:days) { 1 }
              ...
              agfs_scheme_9_trial_fees_and_uplifts(offence_class: 'A').each do |row|
                describe "Uplifts for #{row[:advocate_type]} and offence #{row[:offence_class]}" do
                  let(:advocate_type) { row[:advocate_type] }
                  let(:offence_class) { row[:offence_class] }
                  let(:basic_fee) { row[:basic_fee].to_f }
                  ...
                  [0, 50].each do |quantity|
                    context "when #{quantity} ppe does NOT apply uplift" do
                      let(:ppe) { quantity }
                  ...
                  context 'with 51 to 10,000' do
                    [51, 100, 10_000].each do |quantity|
                      context "when #{quantity} ppe applies uplift" do
                        let(:ppe) { quantity }
                  ...
                  context 'with upper limit 10,000' do
            ...
            describe 'prosecution witnesses (pw)' do
              ...
              agfs_scheme_9_trial_fees_and_uplifts(offence_class: 'A').each do |row|
                describe "Uplifts for #{row[:advocate_type]} and offence #{row[:offence_class]}" do
                  ...
                  context 'when 0 to 10 returns basic fee' do
                  ...
                  # fee appropriate to the advocate type
                  # with no maximum.
                  context 'when 11+ returns basic fee + witness uplift multiplier' do
                    [11, 100, 1000].each do |quantity|
                      context "when #{quantity} pw applies uplift" do
                        let(:pw) { quantity }
          ...
          describe 'modifier-types' do
            ...
            describe 'number of cases' do
              context 'when case 1 carries no uplift' do
                let(:number_of_cases) { 1 }

                it { is_expected.to eql basic_fee }
              end

              context 'when cases 2+ carry 20% uplift of basic fee per case' do
            ...
            describe 'number of defendants' do
              ...
              context 'when defendants 2+ carry 20% uplift of basic fee per defendant' do
