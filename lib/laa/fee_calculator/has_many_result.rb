    ...
    module HasManyResult
      ...
      module ClassMethods
        def has_many_results(association)
          ...
          define_method("#{association}_endpoint".to_sym) do |scheme_pk=nil|
            scheme_pk.nil? ? "#{association.to_s.gsub('_','-')}/" : "fee-schemes/#{scheme_pk}/#{association.to_s.gsub('_','-')}/"
          end

          define_method(association) do |scheme_pk = nil, **options|
            scheme_pk = scheme_pk || options&.fetch(:scheme_pk, nil) || self.fee_scheme
            endpoint = self.send("#{association}_endpoint", scheme_pk)
            json = get(endpoint).body
